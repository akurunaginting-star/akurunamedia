...
def length_issues(article):
    """Only repair length constraints; malformed data still fails validation."""
    ...
    words = sum(len(p.split()) for p in paragraphs)
    if not 120 <= words <= 250:
        issues.append(f"Artikel berisi {words} kata; wajib 120–250, target 140–170 kata.")
    if len(paragraphs[0].strip()) > 400:
        issues.append(f"Pembuka {len(paragraphs[0].strip())} karakter; maksimal 400, target 250–350 karakter.")
    return issues


def generate_with_length_repair(model, instructions, value, schema):
    article = structured(model, instructions, value, schema, "news_package")
    issues = length_issues(article)
    if issues:
        print("Perbaikan panjang artikel: satu percobaan.", flush=True)
        article = structured(model, instructions + "\nPerbaiki panjang draft berdasarkan validation_errors. "
            "Pertahankan fakta, atribusi, tingkat kepastian, dan ID bukti sumber. "
            "Kembalikan seluruh paket JSON. Jangan memotong kalimat di tengah.",
            {**value, "draft_to_repair": article, "validation_errors": issues},
            schema, "news_package_length_repair")
        remaining = length_issues(article)
        if remaining:
            raise JobError("Perbaikan panjang satu kali belum berhasil: " + "; ".join(remaining))
    return article
...
Jangan memberi anjuran investasi personal, janji keuntungan, clickbait, atau mengubah format JSON."""
    a=generate_with_length_repair(config["text_model"],instructions,{"source_name":urlsplit(item["url"]).hostname,"source_url":item["url"],"source_published":item["published"],"source_text":source,"allowed_quotes":quote_lookup},schema)
...
def safe_job_error(error):
    """Expose our bounded JobError messages, never unexpected exception bodies."""
    if not isinstance(error, JobError):
        return "Kesalahan internal; lihat error_type."
    message = str(error)
    for name in ("OPENAI_API_KEY", "AUTO_RUN_KEY", "AGENT_ADMIN_KEY",
                 "WP_CLIENT_SECRET", "WP_ACCESS_TOKEN"):
